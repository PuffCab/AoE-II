...
import Delete1 from '../../Images/Icons/x.png'

function ChatUnits({ detailedUnit }) {
    ...
    const getComments = async () => {
        const q = query(collection(db, "Comments", "Units", `Unit id-${detailedUnit.id}`), orderBy("date"));
    ...
    const handleSubmitComment = async () => {
        try {
            const docRef = await addDoc(collection(db, "Comments", "Units", `Unit id-${detailedUnit.id}`), {
                text: comment,
                date: new Date(),
                author: user.email,
            });
                  ...
                  if (comment.author === user.email) {
                      return (
                          <div key={index} className="ownComments">
                              <img className="" src={Delete} alt="Delete" title="Delete your comment"></img>
