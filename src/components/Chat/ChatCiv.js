...
function ChatCiv({ detailedCivilization }) {
    ...
    const getComments = async () => {
        const q = query(collection(db, "Comments", "Civilizations", `Civilization id-${detailedCivilization.id}`), orderBy("date"));
    ...
    const handleSubmitComment = async () => {
        try {
            const docRef = await addDoc(collection(db, "Comments", "Civilizations", `Civilization id-${detailedCivilization.id}`), {
                text: comment,
                date: new Date(),
                author: user.email,
            });
